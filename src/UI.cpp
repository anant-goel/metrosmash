...
void UI::drawCrosshair(sf::RenderWindow& w) {
    ...
    sf::RectangleShape h({len, 2}); h.setFillColor(c);
    h.setPosition(cx + gap, cy - 1);
    w.draw(h);
    h.setPosition(cx - gap - len, cy - 1);
    w.draw(h);

    sf::RectangleShape v({2, len}); v.setFillColor(c);
    v.setPosition(cx - 1, cy + gap);
    w.draw(v);
    v.setPosition(cx - 1, cy - gap - len);
    w.draw(v);
}
...
void UI::drawExplosiveBar(sf::RenderWindow& w, int count) {
    ...
    for (int i = 0; i < 10; i++) {
        sf::RectangleShape box({18, 24});
        box.setPosition(x + i * 22.f, y);
        box.setOutlineThickness(1.f);
        box.setOutlineColor(sf::Color(200,60,60));
        if (i < count)
            box.setFillColor(sf::Color(220,50,50,220));
        else
            box.setFillColor(sf::Color(50,20,20,150));
        w.draw(box);
    }
}
...
void UI::drawVehicleInfo(sf::RenderWindow& w, const World& world) {
    ...
    for (const auto& v : world.vehicles) {
        ...
        float spd = std::abs(v->speed);
        std::ostringstream oss;
        oss << "SPEED: " << (int)(spd * 3.6f) << " km/h";
...
void UI::drawControls(sf::RenderWindow& w, const World& world) {
    ...
    if (world.player.mode == PlayerMode::ON_FOOT) {
        lines = {"WASD - Move", "Mouse - Look",
                 "Space - Jump", "F - Enter Vehicle",
                 "E - Place Explosive", "Q - Detonate All",
                 "R - Rebuild City"};
    } else {
        lines = {"WASD - Drive", "Mouse - Camera",
                 "F - Exit Vehicle",
                 "E - Place Explosive", "Q - Detonate All"};
    }

    for (int i = 0; i < (int)lines.size(); i++) {
        auto t = makeText(lines[i], sz, c);

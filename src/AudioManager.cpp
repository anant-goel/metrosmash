// SFML/Audio.hpp is ONLY included here, never in the header.
// This prevents miniaudio.h from poisoning <cmath> across the whole project.
...
struct AudioManagerImpl {
    ...
    sf::SoundBuffer* getBuffer(const std::string& filename) {
        ...
        sf::SoundBuffer buf;
        if (buf.loadFromFile(assetDir + "/" + filename)) {
            buffers[filename] = std::move(buf);
            return &buffers[filename];
        }
        std::cerr << "AudioManager: failed to load " << assetDir + "/" + filename << "\n";
        return nullptr;
    }
...
bool AudioManager::init(const std::string& dir) {
    ...
    for (auto& n : {"explosion_large","explosion_medium","c4_detonate",
                    "tank_shoot","building_break","glass_break","car_explode","ui_click"})
        if (impl->nameMap.count(n)) impl->getBuffer(impl->nameMap[n]);
    return true;
}
